...
> 进度前提（2026-09-17）：B00–B06 已实现并经历两轮审查。B04 三级级联阴影默认已回退为单级联（`shadowCascades:1, shadowSize:4096`）。
...
> **✅ 已完成（2026-09-17）**。完整结果、5 个修复缺陷与保留项见 [B07_COMPLETION.md](B07_COMPLETION.md)
> 与[主计划 B07 结果表](STAGE1_COMPLETION_PLAN.md)。本节以下文字保留为原始计划对照。
> 两处与计划的差异需注意：
> 1. 计划的「冻结现有命中置信度/边缘角度渐隐」由**不改动** B03 既有实现达成，未新增冻结代码；
>    「SSR 关/开基础 PBR 色差 ≤0.01/≤2%」这一条**未单独测量**，因此主计划该框保持未勾选。
> 2. 计划的「水面/积水」在本批落为**不透明**水面/积水表面（走材质通道）；
>    半透明水面由 B03 的 `check-transparent-families.cjs` 覆盖并已验证，B07 不重复声称。
