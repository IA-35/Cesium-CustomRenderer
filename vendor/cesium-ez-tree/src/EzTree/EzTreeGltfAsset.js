...
function imageRecordKey(record) {
  ...
  return `${record.mimeType}:${record.bytes.length}:${hash >>> 0}`;
}

function imageBytesEqual(left, right) {
  if (left.mimeType !== right.mimeType || left.bytes.length !== right.bytes.length) return false;
  for (let i = 0; i < left.bytes.length; i++) if (left.bytes[i] !== right.bytes[i]) return false;
  return true;
}

function canonicalizeImageRecords(records, keyOf = imageRecordKey) {
  const buckets = new Map();
  const unique = [];
  let sharedCount = 0;
  for (const record of records) {
    for (const imageRecord of record.imageRecords) {
      const key = keyOf(imageRecord);
      let bucket = buckets.get(key);
      if (!defined(bucket)) { bucket = []; buckets.set(key, bucket); }
      const canonical = bucket.find(candidate => imageBytesEqual(candidate, imageRecord));
      if (!defined(canonical)) {
        bucket.push(imageRecord);
        unique.push(imageRecord);
        continue;
      }
      sharedCount++;
      imageRecord.canonical = canonical;
      for (const primitiveRecord of record.primitiveRecords) {
        if (primitiveRecord.material.imageRecord === imageRecord) primitiveRecord.material.imageRecord = canonical;
      }
    }
  }
  return { unique, sharedCount };
}
  ...
  if (defined(node.mesh)) {
    ...
    for (let i = 0; i < mesh.primitives.length; i++) {
      ...
      if (
        ...
        result.push({ primitive, lod: node.extras?.ccrLod ?? 0 });
      }
    }
  }
...
function createPrimitiveRecords(record) {
  ...
  return scenePrimitives.map(function ({ primitive, lod }) {
    return {
      lod: lod,
      primitive: primitive,
