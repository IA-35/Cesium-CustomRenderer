...
export async function runBaseline(fixture, options = {}) {
  ...
  const configurations = options.configurations || [
    {
      id: 'baseline',
      title: '现有增强基线（无实验特性）',
      options: { environment: true, clouds: true, cloudGeometry: 'shell', environmentAnimation: false, fog: true, shadows: false, shadowMode: 'native' },
      screenSpaceAO: { enabled: false },
  ...
  for (const configuration of configurations) {
    ...
    for (const shot of BASELINE_SHOTS) {
      ...
      check(`nonBlank:${configuration.id}:${shot.id}`, readback.stats.max > readback.stats.min)
      record.shots.push({
        id: shot.id,
        kind: shot.kind,
        title: shot.title,
      ...
      measurements.captures.push({ configuration: configuration.id, shot: shot.id, kind: shot.kind, hash: record.shots[record.shots.length - 1].hash, stats: readback.stats })
    }
  }
