...
- [ ] 逐条画质验收：卫星底图可见、天空不漂白、树荫明显且稳定、无阴影浮空/重影、SSR旋转不骤黑、高空云不扩距、AA不模糊抖动、玻璃水与雾正确排序。
      **未做**：未按 B12 清单逐条验收。部分已被既有检查覆盖（`check-white-tiles` 的 SSR 有效性、B03 `check-transparency-content` 的排序与 classification、B04 `check-shadow-cascades` 的稳定性），但**未逐条复验并出具结论**。
- [ ] 检查拾取/选中、瓦片加载、缩放/旋转、近远切换及多个独立集成用例的场景交互；不恢复RuoYi登录全量验收。
      **未做**：拾取在 B03/B07 已覆盖、瓦片加载在 B04–B06 已覆盖，但**未在 B12 矩阵下统一复验**。
