...
import { selectTreeLod } from '../../../../src/instances/lodSelector.js';
...
function getInstanceRadius(instance, scale) {
  if (instance.kind === "externalTree") return instance.radius * Math.max(scale.x, scale.y, scale.z);
...
function createCommand(options) {
  ...
  const command = new DrawCommand({
    ...
    receiveShadows: primitive.castShadows,
  });
...
function createCommandRecord(options) {
  return {
    geometryLod: options.geometryLod,
    lodGroup: options.lodGroup,
...
function groupGrassAssets(instances, assets) {
  const definitions = new Map((assets ?? []).map((asset) => [asset.id, asset]));
  const groups = new Map();
  for (let i = 0; i < instances.length; i++) {
    const instance = instances[i];
    if (instance.kind !== "grass") continue;
    const asset = definitions.get(instance.asset);
    if (!defined(asset)) throw new Error(`Unknown grass asset: ${instance.asset}`);
    let group = groups.get(asset.id);
    if (!defined(group)) {
      group = { asset: asset, instances: [], alphaCutoff: 0.35 };
...
function addGltfAssetCommandRecords(options) {
  ...
  const external = options.externalAsset;
  for (let i = 0; i < assetRecord.primitives.length; i++) {
    ...
    cellGroups.forEach(function (cellGroup) {
      let lodGroup;
      if (external) {
        const key = `${external.id}:${cellGroup.cellX}:${cellGroup.cellY}`;
        lodGroup = primitive._externalLodGroups.get(key);
        if (!lodGroup) {
          lodGroup = { key, currentLod: 3, height: external.height, sphere: computeBoundingSphere(primitive, cellGroup.instances), count: cellGroup.instances.length };
          primitive._externalLodGroups.set(key, lodGroup);
        }
      }
      addCommandRecord(
        resources,
        createCommandRecord({
          ...
          geometryLod: external ? assetPrimitive.lod : undefined,
          lodGroup,
...
function addGroundAssetRecords(primitive, context, resources) {
  for (const asset of primitive.treeAssets) {
    addGltfAssetCommandRecords({primitive, context, resources,
      assetRecord: resources.gltfAssetMap.get(asset.id),
      instances: primitive._instances.filter(i => i.kind === "externalTree" && i.asset === asset.id),
      colorMode: { fallbackColor: Color.WHITE }, cutoutType: CUTOUT_NONE,
      maximumDistance: Number.POSITIVE_INFINITY, minimumLodRatio: 1,
      externalAsset: asset });
  }
  const grassInstances = collectInstances(primitive._instances, "grass");
  if (grassInstances.length > 0 && primitive.grassAssets.length > 0) {
    const groups = groupGrassAssets(grassInstances, primitive.grassAssets);
    groups.forEach(function (group, assetId) {
      addGltfAssetCommandRecords({
        primitive: primitive,
        resources: resources,
        context: context,
        assetRecord: resources.gltfAssetMap.get(assetId),
        instances: group.instances,
        colorMode: { property: "color", fallbackColor: Color.LIME },
  ...
  } else if (grassInstances.length > 0) {
...
function processGltfAssetRecords(primitive, frameState) {
  ...
  if (!resources.imageRecordsCanonicalized) {
    if (assetRecords.some(asset => !defined(asset.record.gltf) && !defined(asset.record.error))) return false;
    const shared = canonicalizeImageRecords(assetRecords.map(asset => asset.record));
    resources.sharedImageRecords = shared.unique;
    resources.sharedImageCount = shared.sharedCount;
    resources.imageRecordsCanonicalized = true;
  }
  let ready = true;
...
function buildResources(primitive, context) {
  ...
  const resources = {
    ...
    imageRecordsCanonicalized: false,
    sharedImageRecords: [],
    sharedImageCount: 0,
    records: [],
    tiles: undefined,
  ...
  primitive._externalLodGroups.clear();
  for (const asset of primitive.treeAssets) pushGltfAssetRecord(resources, asset.id, asset.url, false);
  const treeGroups = groupTreeInstances(primitive._instances);

  treeGroups.forEach(function (instances, presetName) {
    const options = primitive.treePresets[presetName] ?? loadPreset(presetName);
  ...
  if (grassInstances.length > 0) {
    if (primitive.grassAssets.length > 0) {
      const groups = groupGrassAssets(grassInstances, primitive.grassAssets);
      groups.forEach(function (group, assetId) {
        pushGltfAssetRecord(resources, assetId, group.asset.url, true);
      });
    } else {
...
function EzTreePrimitive(options) {
  ...
  this.treeAssets = options.treeAssets ?? [];
  this.grassAssets = options.grassAssets ?? [];
  this.treePresets = options.treePresets ?? {};
  this.castShadows = options.castShadows ?? false;
  this._externalLodGroups = new Map();
  this._externalThresholds = options.geometryLodThresholds ?? [180, 80, 28];
...
EzTreePrimitive.prototype.update = function (frameState) {
  ...
  // Geometry LOD replaces density thinning for surveyed trees. Decide once in
  // the main view; all color and light passes share the chosen level.
  const pixelHeight = frameState.context.drawingBufferHeight;
  const fovy = frameState.camera.frustum.fovy;
  for (const group of this._externalLodGroups.values()) {
    const d = Math.max(1, Cartesian3.distance(frameState.camera.positionWC, group.sphere.center));
    const pixels = group.height * pixelHeight / (2 * d * Math.tan(fovy / 2));
    group.currentLod = selectTreeLod({ pixels, currentLod: group.currentLod, thresholds: this._externalThresholds });
  }
  this._shadowBuilds = 0;
  ...
  const processVisibleRecord = (record) => {
    if (record.lodGroup && record.geometryLod !== record.lodGroup.currentLod) return;
    ...
    this._submittedRecords.push(record);
  };
 ...
 */
// CCR light traversal: use main-camera geometry LOD, not a second camera decision.
EzTreePrimitive.prototype.updateForPass = function (frameState, passState) {
  const resources = this._resources;
  // Cesium also calls this hook for tile preload/pick passes without a list.
  if (!Array.isArray(passState?.commandList)) return;
  if (!this.show || !this.castShadows || !resources?.gltfRecordsBuilt) return;
  for (const record of resources.records) {
    if (record.lodGroup && record.geometryLod !== record.lodGroup.currentLod) continue;
    if (passState.cullingVolume.computeVisibility(record.boundingSphere) === Intersect.OUTSIDE) continue;
    if (!record.command) {
      if (this._shadowBuilds >= this.maximumCommandBuildsPerFrame) continue;
      this._shadowBuilds++;
      if (!processCommandRecordResources(record, this, frameState, resources.shaderProgram)) continue;
    }
    record.lastUsedFrame = frameState.frameNumber;
    passState.commandList.push(record.command);
  }
};
