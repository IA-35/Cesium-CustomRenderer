...
环境模块默认随 VisualPipeline 启用。使用 Cesium 的真实太阳、Rayleigh/Mie 大气与 PBR 环境贴图，增加自有线性 HDR 体积合成。没有修改静态发行包或材质；没有实现 SSRT/GI、全球球壳云或投到地面的云影。
...
- 旧版南湖材质兼容：仅识别 `SM_NH_Terr/tileset.json` 和 `SM_NH_Building/tileset.json`。这些已核实资产的材质统一使用0.45粗糙度，环境开启时将默认IBL因子从(1,1)调整为(1,0)，保留天空漫反射，避免大面积天空镜面反射在平视时洗白纹理。显式IBL设置、已有CustomShader、其他资产不套用此规则；停用环境时恢复原因子，外部接管的设置不被覆盖。原模型文件不变。
...
地面高度雾与高云分别积分，再按相机高度排序合成。这是分层、单次散射近似，不能称为完整多次散射或任意互相穿插介质。云影投地、云在IBL/反射探针中的反馈、局部灯光体积散射尚未实现。空间体积相对校区锚点的ENU平面定义；相机超过锚点100km时旁路体积，继续使用原生全球大气，不能称为全球体积云。
...
需要depthTexture、floatingPointTexture、colorBufferFloat，以及3D透视单视锥。真正多视锥、正交视图、地下相机、透明Globe和区域外旁路体积。BLEND/OIT一般不写深度，雾的遮挡仍可能取透明物后方表面，尚未做玻璃分层合成。关闭地形深度测试时，地表后的普通primitive仍可能被原生渲染到前面；最近距离合并对这类覆盖物的雾量是保守近似。上下文丢失后的自动重建、全业务页面及所有硬件兼容性未完成认证。
