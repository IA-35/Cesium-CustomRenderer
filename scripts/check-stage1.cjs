...
;(async () => {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${process.env.CCR_TEST_PORT || 8877}/tests/rendering/stage1-fixture.html?mode=umd`)
    await page.waitForFunction(()=>window.fixture)
    result.umd=await page.evaluate(async()=>{
      const {runStage1SceneChecks}=await import('/tests/rendering/stage1-scene.js')
      return runStage1SceneChecks(fixture)
    })
    await page.goto(`http://127.0.0.1:${process.env.CCR_TEST_PORT || 8877}/examples/stage1.html`)
    ...
    fs.writeFileSync(path.join(output, 'stage1-gpu.json'), JSON.stringify({ ...result, errors }, null, 2))
    console.log(JSON.stringify({checks:Object.fromEntries(['ssao','hbao','bloom','shell','noise','scene','umd'].map(key=>[key,Object.keys(result[key].checks).length])),noise:result.noise,resize:result.resize,gpu:result.profile.gpu,errors}))
