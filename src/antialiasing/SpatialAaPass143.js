...
import { spatialQuality } from './settings143.js'
import { fxaaShader } from './fxaaShader143.js'
...
export default class SpatialAaPass143 {
  constructor(C, scene, algorithm = 'smaa') {
    if (!/^1\.143(?:\.0)?$/.test(C.VERSION)) throw new Error('SpatialAaPass143 requires Cesium 1.143')
  ...
  setQuality(value) {
    if (!['sharp', 'balanced', 'smooth'].includes(value) || value === this.quality || this._destroyed) return
  ...
  setEnabled(value) {
    ...
    this._setFxaa(true)
    if (!this._supported()) {
      this.error = `${this.algorithm.toUpperCase()} requires WebGL 2`
      this._requestRender()
      return
    }
  ...
  _createStages() {
    const C = this.C
    const prefix = `${this.algorithm}143_${++nextId}`
    ...
    const quality = spatialQuality(this.quality)
    if (this.algorithm === 'fxaa') {
      const fxaa = createStage('fxaa', fxaaShader(C, quality))
      this.stages = { fxaa }
      this.composite = new C.PostProcessStageComposite({ name: prefix, stages: [fxaa], inputPreviousStageTexture: false })
      this._createCollection()
      return
    }
    const edges = createStage('edges', edgesShader.replace('#define SMAA_THRESHOLD 0.1', '#define SMAA_THRESHOLD ' + quality.smaaThreshold))
    const weights = createStage('weights', weightsShader.replace('#define SMAA_MAX_SEARCH_STEPS 8', '#define SMAA_MAX_SEARCH_STEPS ' + quality.smaaSearchSteps), {
  ...
  async _loadLookups(generation) {
    if (this.algorithm === 'fxaa') { this.ready = true; this._setFxaa(false); return true }
    const C = this.C
    try {
  ...
  _execute(context, color, depth, id) {
    ...
    try {
      this._withViewport(context, () => {
        ...
        for (const stage of [this.stages.edges, this.stages.weights].filter(Boolean)) {
  ...
  getDiagnostics() {
    ...
    return {
      ...
      effective: this.enabled && valid && !this.scene.postProcessStages.fxaa.enabled ? this.algorithm
        : this.enabled && this.scene.postProcessStages.fxaa.enabled ? 'fxaa' : 'off',
      error: this.error, quality: this.quality, settings: spatialQuality(this.quality),
      ...
      resourceCounts: { collections: this.collection ? 1 : 0, stages: this.stages ? Object.keys(this.stages).length : 0,
